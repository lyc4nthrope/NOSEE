...
import { BORDER } from '../adminStyles';
  ...
  return (
    <div role="presentation" className="admin-modal-overlay" style={{
      ...
      <div ref={modalRef} role="dialog" aria-modal="true" aria-labelledby="ban-modal-title" style={{
        background: 'var(--bg-surface)', border: `1px solid ${BORDER}`, borderRadius: 14,
        padding: '28px 32px', width: 420, maxWidth: '90vw',
      }}>
        <h2 id="ban-modal-title" style={{ margin: '0 0 8px', fontSize: 'var(--admin-fs-xl)', fontWeight: 700, color: 'var(--text-primary)' }}>
          {isBanning ? td.banTitle : td.unbanTitle}
        </h2>
        <p style={{ margin: '0 0 20px', fontSize: 'var(--admin-fs-md)', color: 'var(--text-secondary)' }}>
          {isBanning
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button
            onClick={onCancel}
            style={{ background: 'none', border: `1px solid ${BORDER}`, color: 'var(--text-secondary)', borderRadius: 8, padding: '8px 18px', fontSize: 'var(--admin-fs-base)', cursor: 'pointer' }}
          >
            {td.cancelBtn}
          </button>
          <button
            ...
            style={{
              ...
              borderRadius: 8, padding: '8px 18px', fontSize: 'var(--admin-fs-base)', cursor: 'pointer', fontWeight: 600,
            }}
          >
